...
#define DECLARE_TIMING(s)                                                      \
    std::chrono::time_point<std::chrono::high_resolution_clock> timeStart_##s; \
    std::chrono::duration<double> timeDiff_##s;                                \
    double timeTally_##s = 0;                                                  \
    long countTally_##s = 0
#define START_TIMING(s) timeStart_##s = std::chrono::high_resolution_clock::now()
#define STOP_TIMING(s)                                                          \
    timeDiff_##s = (std::chrono::high_resolution_clock::now() - timeStart_##s); \
    timeTally_##s += timeDiff_##s.count();                                      \
    countTally_##s++
